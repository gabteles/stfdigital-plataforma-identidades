...
public class GrupoRepositoryImpl extends SimpleJpaRepository<Grupo, GrupoId> implements GrupoRepository {
    ...
    public Grupo findOne(String nome, TipoGrupo tipo) {
        TypedQuery<Grupo> query = entityManager
                .createQuery("FROM Grupo grupo WHERE grupo.nome = :nome AND grupo.tipo = :tipo", Grupo.class);
    ...
    public List<Recurso> findRecursoByGrupo(GrupoId id) {
        TypedQuery<Recurso> query = entityManager.createQuery(
                "SELECT recu FROM Grupo grupo INNER JOIN grupo.recursos recu WITH grupo.id = :id", Recurso.class);
    ...
    public GrupoId nextId() {
        Query query = entityManager.createNativeQuery("SELECT identidades.seq_grupo.NEXTVAL FROM DUAL");
