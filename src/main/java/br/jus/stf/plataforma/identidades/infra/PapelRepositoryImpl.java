...
public class PapelRepositoryImpl extends SimpleJpaRepository<Papel, PapelId> implements PapelRepository {
    ...
    public List<Recurso> findRecursoByPapel(PapelId id) {
        TypedQuery<Recurso> query = entityManager.createQuery(
                "SELECT recu FROM Papel papel INNER JOIN papel.recursos recu WITH papel.id = :id", Recurso.class);
    ...
    public PapelId nextId() {
        Query query = entityManager.createNativeQuery("SELECT identidades.seq_papel.NEXTVAL FROM DUAL");
