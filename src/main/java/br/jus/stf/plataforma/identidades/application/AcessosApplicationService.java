...
public class AcessosApplicationService {
    ...
    public void handle(ConfigurarPermissoesUsuarioCommand command) {
        ...
        Optional.ofNullable(command.getPapeisRemovidos())
                .ifPresent(p2 -> p2.forEach(p -> papeisRemov.add(new PapelId(p))));
        Optional.ofNullable(command.getGruposRemovidos())
                .ifPresent(g2 -> g2.forEach(g -> gruposRemov.add(new GrupoId(g))));
        Optional.ofNullable(command.getRecursosRemovidos())
                .ifPresent(r2 -> r2.forEach(r -> recursosRemov.add(recursoRepository.findOne(new RecursoId(r)))));
        Optional.ofNullable(command.getPapeisAdicionados())
                .ifPresent(p1 -> p1.forEach(p -> papeisAdic.add(papelRepository.findOne(new PapelId(p)))));
        Optional.ofNullable(command.getGruposAdicionados())
                .ifPresent(g1 -> g1.forEach(g -> gruposAdic.add(grupoRepository.findOne(new GrupoId(g)))));
        Optional.ofNullable(command.getRecursosAdicionados())
                .ifPresent(r1 -> r1.forEach(r -> recursosAdic.add(recursoRepository.findOne(new RecursoId(r)))));
